<ul class="uk-nav uk-nav-side uk-nav-parent-icon" data-uk-nav data-uk-sticky="{top:60}">
    ...
    @foreach($resources as $resource)
        <li class="{{ $segments[1] == $resource->slug ? 'uk-active' : '' }}"><a href="{{ url('admin/dashboard') }}" class="uk-text-large"><i class="{{ $resource->icon }} uk-icon-justify"></i>{{ $resource->title }}</a></li>
    @endforeach
